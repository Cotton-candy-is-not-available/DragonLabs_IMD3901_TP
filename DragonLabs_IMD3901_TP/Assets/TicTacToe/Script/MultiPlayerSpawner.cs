...
public class MultiPlayerSpawner : NetworkBehaviour
{
    ...
    [Header("------------ Prefabs -------------")]
    public NetworkObject[] X_Piece;//prefab with network object attached
    public NetworkObject[] O_Piece;//prefab with network object attached
    ...
    void Update()
    {
        ...
        if (turn.Value == 1 )//if player 1 turn
        {
            spawnX_PieceServerRpc();//spawn the ball infornt of player 1
    ...
    void spawnX_PieceServerRpc()//allows client to also spawn/ see spawned ball
    {

        foreach (NetworkObject X in X_Piece)
        {

            if (!IsServer) return;
            NetworkObject newX_piece = Instantiate(X, SpawnPosistion.position, Quaternion.identity);
